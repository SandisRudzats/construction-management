<template>
  <div class="view-construction-sites">
    ...
    <div v-else-if="error || employeeStore.getError">Error: {{ error || employeeStore.getError }}</div>
      ...
      <div v-for="site in constructionSites" :key="site.id" class="site-container">
        ...
        <div class="site-details" v-if="editingSiteId !== site.id">
          ...
          <div v-if="userStore.user.role === 'admin'" class="detail-row">
            ...
            <button @click="deleteSite(site.id)" class="action-button delete-button">Delete Site</button>
          </div>
        </div>
        <div class="site-edit" v-else>
          ...
          <div class="detail-row">
            <strong>Required Access Level:</strong> <input type="number" v-model="editedSite.required_access_level" />
          </div>
          <button @click="updateSite(site)" class="action-button">Update Site</button>
        </div>

        <h4>Work Tasks</h4>
        <button v-if="userStore.user.role === 'admin' || userStore.user.id === site.manager_id" @click="toggleAddTask(site)" class="action-button add-button">
        ...
        <div v-if="site.showAddTask">
          <div class="detail-row">
            ...
            <select v-model="newTask.employee_id">
              <option v-for="employee in getFilteredEmployees(site.manager_id, site.required_access_level)" :key="employee.id" :value="employee.id">
                {{ employee.first_name }} {{ employee.last_name }}
              </option>
              <option v-if="getFilteredEmployees(site.manager_id, site.required_access_level).length === 0" disabled>
        ...
        <table class="work-tasks-table">
          ...
          <tr v-for="task in site.workTasks" :key="task.id">
            ...
            <td>
              <button
                v-if="editingTaskId !== task.id && (userStore.user.role === 'admin' || userStore.user.id === task.employee_id)"
                @click="editTask(task)"
                class="action-button"
              >
                Edit
              </button>
              <button v-else-if="editingTaskId === task.id" @click="updateTask(site.id, task)" class="action-button save-button">Save</button>
              <button @click="deleteTask(site.id, task.id)" class="action-button delete-button">Delete Task</button>
            </td>
          </tr>
          </tbody>
        </table>
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchConstructionSites = async () => {
      ...
      try {
        ...
        if (userStore.user.role === 'admin') {
          ...
          response = await api.get('v1/construction-site/my-sites');
        } else {
          ...
          for (const site of constructionSites.value) {
            site.workTasks = taskResponse.data.filter(task => task.construction_site_id === site.id);
          }
          loading.value = false;
    ...
    const getFilteredEmployees = computed(() => (siteManagerId: number, requiredAccessLevel: number) => {
      const filtered = employeeStore.getEmployees.filter(employee => employee.manager_id === siteManagerId && employee.access_level >= requiredAccessLevel);
      return filtered;
    });

    onMounted(async () => {
      // Fetch employees without a siteManagerId initially to load all employees.
